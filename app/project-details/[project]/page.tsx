...
export default function ClientProjectDetails() {
  ...
  useEffect(() => {
    ...
    const fetchingFunction = async () => {
      ...
      if (ServerResponse.status == 400) {
        ...
        const filterProject = ServerResponse.find(
          (items: ProjectType) =>
            items.project_name.toLowerCase() === decodeURI(project).toLowerCase()
        );
  ...
  async function handleDeleteTask(taskId: string) {
    ...
    try {
      const res = await fetch("/API/cli/task/deleting-task", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    <>
      ...
      <div className="project-details-container ml-[400px] pt-[150px]">
        <div className="project-name text-[18px] text-[#747474]">
          <p>
            Project Details / {decodeURI(project)} /{" "}
            <button
              onClick={() => window.location.href = `/edit-project/${linkId?.id}` || "#"}
          ...
          {array?.length === 0 ? (
            <div className="flex justify-center items-center align-center mt-[300px]">
              <div className="message text-center">
                <h1 className="text-[40px] font-extrabold text-[#aaaaaa]">
                  No task found!
                </h1>
                ...
                <button
                  ...
                  className="mt-[10px] h-[35px] w-[150px] bg-blue-500 cursor-pointer text-[white]"
                >
                  Add task now
                </button>
              </div>
            ...
            array?.map((items, index) => {
              return (
                <div
                  key={index}
                  className="each-table-card border-l-5 border-blue-500 mt-[50px] w-[90%] cursor-pointer rounded-[10px] h-[auto] pb-[50px] shadow-[0_0_5px_#747474]"
                >
                  <div className="inner-part pl-[20px] pt-[20px]">
                    ...
                    <div className="button-card-section mt-[20px] flex flex-wrap">
                      <button
                        onClick={() =>
                          (window.location.href = `/edit-task/${decodeURI(project)}/${items.id}`)
                        }
                        className="w-[150px] h-[40px] bg-yellow-500 cursor-pointer text-white"
                      >
                        Edit Task
                      </button>
                      <button
                        onClick={() => handleDeleteTask(items.id)}
                        className="ml-[20px] w-[150px] h-[40px] bg-red-500 cursor-pointer text-white"
                      >
                        Delete Task
                      </button>
                    </div>
